...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  4 22:55:05 2017
...
file_location2 = "/Users/wangtianpei/Desktop/python/Week1hourly.xlsx"
...
for row in range(sheet2.nrows-1):
    ...
    start_com.append(sheet2.cell_value((row+1),3))
    end_com.append(sheet2.cell_value((row+1),4))
...
for i in range(sheet2.nrows-2):
    if time_stamp[i+1] != time_stamp[i]:
        Tim = Tim+1
    if time_stamp[i+1] != time_stamp[i] and Tim-1==534:  #376
        previous_row = i+1
    if time_stamp[i+1] != time_stamp[i] and Tim-1>534:
        m = folium.Map(
        ...
        save_map(m,filename,pngname)    #save m here
